 ...
 * 실 컨트롤러 25개는 전부 정상이라 아무것도 실패하지 않으므로, 위반 하나를 일부러 만들어 둔다.
 *
 ...
 *
 * <p>{@code @RestController} 가 아니라 {@code @Controller + @ResponseBody} 인 것도 의도다. 규칙이 메타 어노테이션
 * ({@code @Controller}·{@code @RequestMapping})으로 판정하는지 고정한다 — 직접 어노테이션만 보면 이 우회가 통과한다.
 */
@Controller
