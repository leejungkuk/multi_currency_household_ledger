...
public enum ExchangeErrorCode implements ErrorCode {
    ...
    EXCHANGE_API_ERROR(
            "EXCHANGE_API_ERROR", "환율 정보를 가져오는데 실패했습니다.", HttpStatus.INTERNAL_SERVER_ERROR),
    EXCHANGE_API_AUTH_ERROR(
            "EXCHANGE_API_AUTH_ERROR", "수출입은행 API 인증키 오류", HttpStatus.INTERNAL_SERVER_ERROR),
    EXCHANGE_API_LIMIT_EXCEEDED(
            "EXCHANGE_API_LIMIT_EXCEEDED", "수출입은행 API 일일 호출 한도 초과", HttpStatus.SERVICE_UNAVAILABLE),
